...
<script type="text/javascript">
    ...
    function goSubmit() {
		...
		frm.method = "get";
		frm.action = "<%= request.getContextPath() %>/testPerson4.do"; 
		<%-- <%= request.getContextPath() %> 은 
		         컨텍스트 경로명을 알려주는것으로 다이내믹웹프로젝트로 만들면
		         컨텍스트명이  다이내믹웹프로젝트명이 된다.
		         그러므로 여기서는 컨텍스트명이 /MyWeb 이 되어진다. 
		     << 중요 암기!! >>
		         웹경로를 나타낼때 맨앞에 /는  
		         확장자가 .html 이나 .jsp 일 경우에는 
		     ip주소:포트번호이 생략된 것이고,
		         확장자가 .java 이나 .xml 일 경우에는 
		     ip주소:포트번호/컨텍스트명이 생략된 것이다. 
		     
		         그래서 여기서는 확장자가 .jsp 이므로   
		     /MyWeb/testPerson1.do 은 
...
<!-- Form Name -->
<legend><h2 class="text-info">개인 취향 테스트(GET/POST)</h2></legend>
